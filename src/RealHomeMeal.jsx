...
export default function RealHomeMeal() {
  ...
  const [cookDays, setCookDays] = useState(["월", "화", "수", "목", "금"]); // 메뉴(식단) 필요한 날 — 기본 평일 5일 켜짐
  ...
  // 메뉴(식단) 필요한 날 토글 (다중 선택) — 선택된 날에만 식단 생성. 0개면 빈 상태 안내.
  const toggleCookDay = (d) =>
    setCookDays((prev) => (prev.includes(d) ? prev.filter((x) => x !== d) : [...prev, d]));
  ...
  const baseWeekPlan = useMemo(() => {
    // 선택된 "메뉴 필요한 날"만 식단 생성 → 나머지 날은 eatOutDays로 넘겨 "쉬어가는 날"로 처리
    const offDays = DAYS.filter((d) => !cookDays.includes(d));
    const base = generateWeekPlan(MENUS, selected, { eatOutDays: offDays, excludeSpicy, meals });
  ...
  const offDayCount = weekPlan.filter((d) => d.rest).length; // 선택 안 한(쉬어가는) 날
  ...
  const shoppingList = useMemo(() => {
    ...
    weekPlan.forEach((d) => {
      if (d.rest || d.mealkit) return; // 쉬어가는·밀키트 날은 장보기 불필요
          ...
          {tab === "plan" && (generating ? (
            ...
            <div className="flex flex-col items-center justify-center text-center" style={{ minHeight: "55vh" }}>
              ...
              <p className="mt-1 text-[12.5px] leading-snug" style={{ color: C.sub }}>
                ‘식단 설정 → 메뉴 필요한 날’에서 요일을 켜면<br />그 날 식단이 나와요
              </p>
              <button
                onClick={() => setTab("ingredients")}
                className="mt-5 flex items-center justify-center gap-1.5 rounded-2xl px-6 py-3 text-[14px] font-bold"
            ...
            <div className="flex flex-col items-center justify-center text-center" style={{ minHeight: "55vh" }}>
              ...
              <p className="mt-1 text-[12.5px]" style={{ color: C.sub }}>
                담은 재료 {selected.length}개 · 끼니 {orderedMeals.join("·")} · 요리할 날 {cookDays.length}일
              </p>
...
// 설정 입력 컨트롤 — 끼니 / 메뉴 필요한 날 / 색다른 메뉴 / 고춧가루 제외
//  bare=true면 카드 래퍼 없이 내용만 (이미 Card 안에 들어갈 때)
function PlanControls({ meals, onToggleMeal, cookDays, onToggleCookDay, specialFreq, onSetSpecialFreq, excludeSpicy, onToggleSpicy, bare }) {
  const FREQ_OPTIONS = [
    { key: "none", label: "없음" },
  ...
  return (
    <Wrapper className={bare ? "space-y-3" : "space-y-3 rounded-3xl p-4"} style={wrapStyle}>
      ...
      {/* 메뉴 필요한 날 (요일 선택, 다중) — 켜진 날에만 식단이 나옴 */}
      <div>
        <p className="mb-1.5 text-[11.5px] font-semibold" style={{ color: C.sub }}>
          메뉴 필요한 날 <span style={{ color: C.line }}>·</span> 요일 선택 <span className="font-medium" style={{ color: C.line }}>(켠 날만 식단이 나와요)</span>
        </p>
        <div className="flex gap-1">
          {DAYS.map((d) => {
            const on = cookDays.includes(d);
            return (
              <button
                key={d}
                onClick={() => onToggleCookDay(d)}
  ...
  return (
    <section className="rounded-3xl p-5" style={{ background: C.card, border: `1px solid ${C.gold}40` }}>
      ...
      <div className="flex items-center gap-1.5">
        <span className="h-4 w-1 rounded-full" style={{ background: C.gold }} />
        <h2 className="text-[16px] font-bold tracking-tight" style={{ color: C.ink }}>🛒 장보기</h2>
      </div>

      {/* a) 🧊 냉장고에 있는 것 — 담은 재료 (차분한 색) */}
      ...
      {/* b) 🛒 이번 주 사야 할 것 — 부족 재료 + 쿠팡 검색 (강조 색) */}
      <div>
        <h3 className="text-[13.5px] font-bold" style={{ color: C.ink }}>🛒 이번 주 사야 할 것</h3>
        <p className="mb-2 mt-0.5 text-[11.5px]" style={{ color: C.sub }}>
          {shopItems.length > 0
      ...
      {/* c) ➕ 하나만 더 사면 되는 메뉴 — groups 있을 때만 */}
      {oneMoreGroups.length > 0 && (
        <>
          <Divider />
          <div>
      ...
      {/* 쿠팡 파트너스 고지 — 쿠팡 링크가 노출되는 b·c가 있을 때만 */}
      {(shopItems.length > 0 || oneMoreGroups.length > 0) && (
        <p className="mt-3.5 text-[10.5px] leading-snug" style={{ color: C.sub }}>
